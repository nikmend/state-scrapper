...
class StateSpiderDeep(scrapy.Spider):
    ...
    def parse(self, response):
        if self.totalPropertiesCount == 0:
            ...
            for url in self.start_urls[10:11]:
                print('????? IN NESTED LOOP: ', url)
                for prop in response.css('.detail_wrap'):
                    print('????? IN SUB NESTED LOOP: ', prop.css('.content .header .data-details-id::attr(href)').extract()[0])
                    yield scrapy.Request(url=prop.css('.content .header .data-details-id::attr(href)').extract()[0],
                                         method='GET', callback=self.parse, headers=self.headers)

        else:
            print('Im out bitch')
            print(response.css('.m_property_info_title').extract())
            print(response.css('#latitude::attr(value)').extract()[0])
            print(response.css('#longitude::attr(value)').extract()[0])
            print(response.css('.link-whatsapp-contact::attr(onclick)').extract()[0][27:].split('?')[0])
            #TODO: map old values to new page :
